...
class PlacementDb:
    ...
    # ------------------------------------------------------------------ Part 3: safe writes (DONE)

    def create_application(self, student_id: int, drive_id: int) -> tuple[int, bool]:  # DONE (3.1): ON CONFLICT DO NOTHING
    ...
    def claim_slot(self, slot_id: int, student_id: int, expected_version: int) -> bool:  # DONE (3.2): WHERE version = ?
    ...
    def record_notification(self, roll_no: str, message: str, dedupe_key: str) -> tuple[int, bool]:  # DONE (3.3): ON CONFLICT dedupe_key DO NOTHING
        ...
        if cur.rowcount == 1:
            return cur.lastrowid, True
        existing = self.conn.execute(
            "SELECT id FROM notification WHERE dedupe_key = ?", (dedupe_key,),
        ).fetchone()
        return existing["id"], False
    ...
    def once(self, key: str, tool_name: str, effect: Callable[[], dict]) -> tuple[dict, bool]:  # DONE (2.3): run effect at most once per key
        with self.transaction() as c:
            existing = c.execute("SELECT result FROM idempotency WHERE key = ?", (key,)).fetchone()
