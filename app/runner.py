...
def call_tool(tools: PlacementTools, placement: PlacementDb, key: str, name: str, args: dict) -> dict:  # DONE (2.4): side-effects through once()
    ...
    try:
        if name in tools.SIDE_EFFECTS:
            result, _created = placement.once(key, name, lambda: tools.call(name, args))
            return result
...
def execute_run(claimed: Claimed, *, store: RunStore, placement: PlacementDb, tools: PlacementTools, provider,
    ...
    def between_steps() -> str | None:  # DONE (L1): cancel check + heartbeat
