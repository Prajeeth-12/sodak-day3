...
class RunStore:
    ...
    # ================================================================== Part 1: the queue (DONE)

    def enqueue(self, thread_id, text, model, max_attempts=3):  # DONE (1.2): save message + queued run in one transaction
    ...
    def claim_next(self, worker_id: str, lease_seconds: float) -> Claimed | None:  # DONE (1.3): SELECT + UPDATE in one BEGIN IMMEDIATE
        now = self.clock()
        with self.transaction() as c:
            row = c.execute(
            ...
            c.execute(
                ...
                f" attempts = ?, started_at = COALESCE(started_at, {NOW_SQL})"
                f" WHERE id = ?",
                (worker_id, now + lease_seconds, new_attempts, row["id"]),
            )
        return Claimed(row["id"], row["thread_id"], new_attempts)

    def heartbeat(self, run_id: str, worker_id: str, lease_seconds: float) -> bool:  # DONE (1.4): extend lease if still owner
        cur = self.conn.execute(
    ...
    def reap_expired(self) -> list[str]:  # DONE (1.4): dead workers → back to queued or dead
        now = self.clock()
        with self.transaction() as c:
            rows = c.execute(
            ...
            for r in rows:
                if r["attempts"] < r["max_attempts"]:
                    c.execute(
                        ...
                        " lease_owner = NULL, lease_until = NULL, available_at = ?"
                        " WHERE id = ?",
                        (now, r["id"]),
                    )
                else:
    ...
    def request_cancel(self, run_id: str) -> str | None:  # DONE (L1): queued→cancelled, running→set flag
        with self.transaction() as c:
    ...
    def mark_cancelled(self, run_id: str, worker_id: str) -> bool:  # DONE (L1): cancel if running + owner
        cur = self.conn.execute(
            f"UPDATE run SET status = 'cancelled', lease_owner = NULL, lease_until = NULL,"
            f" finished_at = {NOW_SQL}"
            f" WHERE id = ? AND status = 'running' AND lease_owner = ?",
            (run_id, worker_id),
        )
        return cur.rowcount == 1

    # ================================================================== Lab 2: retry and dead-letter (DONE)

    def fail_attempt(self, run_id: str, worker_id: str, error_code: str, retryable: bool,  # DONE (L2): retry+backoff or dead-letter
        ...
        with self.transaction() as c:
            ...
            if row["attempts"] < row["max_attempts"]:
                ...
                return "queued"
            c.execute(f"UPDATE run SET status = 'dead', error_code = ?, lease_owner = NULL, lease_until = NULL,"
                      f" finished_at = {NOW_SQL} WHERE id = ?", (error_code, run_id))
            return "dead"
